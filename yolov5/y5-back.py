        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det)):
                    ...
                    label = f'{names[c]} {i} {j} {conf_percent:.0f}% , W:{width:.0f}, H:{height:.0f}, Zone:{zone}'
                    annotator.box_label(xyxy, label, color=(16776960, True))

                    # Update highest priority
                    if priority < highest_priority or priority == highest_priority:
                        highest_priority = priority
                        alarm_object_details = f'{names[c]} - Priority: {priority}, Zone: {zone}'
                        counter = counter + 1
